...
class Fitness:
    ...
    def __str__(self) -> str:
        if isinstance(self.value, Tensor):
            return str(round(self.value.item(), 5))
...
class FitnessMetric(ABC):
    ...
    def create_fitness_metric(
        ...
        elif metric_name == FitnessMetricName.ARGO:
            class_kwargs = \
                {k: kwargs.get(k, None) for k in ArgoFitnessMetric.__init__.__code__.co_varnames[1:]}
            fitness_metric = ArgoFitnessMetric(**class_kwargs)
        else:
...
class ArgoFitnessMetric(FitnessMetric):
