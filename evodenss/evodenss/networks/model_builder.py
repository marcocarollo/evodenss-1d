...
class ModelBuilder():
    ...
    def __init__(self,
        ...
        if len(input_shape) == 3:
            ...
            self.layer_shapes: dict[InputLayerId, Dimensions1d] = {
                InputLayerId(-1): Dimensions1d(channels=input_shape[0],
                                                  length=input_shape[1])
        }
        else:
            raise ValueError(f"Unexpected input shape found: {input_shape}")
        self.projector_layer_shapes: dict[InputLayerId, Union[Dimensions, Dimensions1d]] = {}
    ...
    def assemble_network(self,
        ...
        try:
            for i, l in enumerate(self.parsed_network.layers):
                ...
                inputs_shapes: dict[InputLayerId, Union[Dimensions, Dimensions1d]] = \
    ...
    def _get_layers_to_fix_shape_mismatches_resnet_approach(self,
        ...
        target_shape: Union[Dimensions, Dimensions1d] = inputs_shapes.pop(minimum_input_id) # side effect: removes element from dict
    ...
    def _create_torch_layer(self, layer: Layer, layer_name: str, is_projector_network: bool=False) -> nn.Module:
        ...
        elif len(inputs_shapes) == 2:
            expected_input_dimensions = Dimensions1d(first_input.channels, first_input.length)
        else:
        ...
        if is_projector_network is False:
            self.layer_shapes[InputLayerId(layer.layer_id)] = \
                getattr(Dimensions if len(inputs_shapes) == 3 else Dimensions1d, 'from_layer')(layer, expected_input_dimensions)
        else:
        ...
        if layer.layer_type == LayerType.CONV:
            ...
            layer_to_add = self._build_convolutional1d_layer(layer, expected_input_dimensions)
    ...
    def _build_dense_layer(self, layer: Layer, layer_name: str, input_dimensions: Union[Dimensions, Dimensions1d]) -> nn.Sequential:
    ...
    def _build_dense_layer1d(self, layer: Layer, layer_name: str, input_dimensions: Dimensions1d) -> nn.Sequential:
        activation = ActivationType(layer.layer_parameters.pop("act"))
        torch_layers_to_add: list[nn.Module] = []
        if layer_name.endswith(f"{LayerType.FC1D.value}{SEPARATOR_CHAR}1"):
            torch_layers_to_add.append(nn.Flatten())
            torch_layers_to_add.append(nn.Linear(**layer.layer_parameters,
                                                 in_features=input_dimensions.flatten(),
                                                 device=self.device.value))
